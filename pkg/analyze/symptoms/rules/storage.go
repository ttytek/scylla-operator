...
func buildNodeConfigSymptoms() symptoms.SymptomTreeNode {
	...
	nodeConfigNonexistentDevice := symptoms.NewSymptom(
		...
		[]string{
			"NodeConfig failed to mount volumes for a pod, because the device that backs the mounts does not exist.",
			"This may be due a NodeConfig misconfiguration.",
		},
		[]string{
			"Make sure that the NodeConfig's 'spec.localDiskSetup' mount points exist.",
			"Verify the CLI arguments of local-csi-driver pods, in particular volumes-dir.",
			"For more information, see the NodeConfig's conditions and local-csi-driver docs: https://github.com/scylladb/local-csi-driver/.",
		},
		selector.
			New().
